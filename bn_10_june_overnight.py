...
prev_day_data = [
    {'NIFTY': {'use_strike': 23250, 'strd_price': 420.9, 'strd_theta': 2643822.774}},
    {'BANKNIFTY': {'use_strike': 49700, 'strd_price': 979.8, 'strd_theta': 8126420.428}},
    {'FINNIFTY': {'use_strike': 22250, 'strd_price': 361, 'strd_theta':  4400634.316}},
    {'MIDCPNIFTY': {'use_strike': 11575, 'strd_price': 201, 'strd_theta':  4895066}}
]
 # NIFTY 22550 614
# MIDCPNIFTY 11450 290
...
# ---------------------------------------------------------------
    ...
    # ----------------------------------------------------------------
    # df_fut['fut'] = df.loc[
    #     (df['Symbol'] == sym) & (df['Expiry'] == fut_expiry) & (df['Opttype'] == 'XX'), 'Close'].values
    val = df.loc[
...
# ----------------------------------------------------------------------------------------------
    ...
    # check_df1 = pd.DataFrame([1,2])
    # check_df2 = pd.DataFrame([1, 2, 3])
    condition = True
    while condition:
        check_df1 = df.loc[
...
# ----------------------------------------------------------------------------------------------
    ...
    for j in range(375):
        ...
        # today_date = datetime.datetime.strptime('07-jun-24', '%d-%b-%y').date()  # fixing 28 may for now
    ...
    df_fut.to_csv(f'test_overnight_{sym}_{today_date}.csv')
    print(f'Overnight CSV file name for {sym} - {today_date}')
    df_fut = df_fut.empty