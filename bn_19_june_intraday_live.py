...
for i in range(len(sym_list)):
    ...
    exp = exp_list[i]
    if sym == 'NIFTY':
        save_to_sheet(sym = 'NIFTY', exp = '20-Jun-24', df=df_nifty, sheet_name='nifty', wb=wb, plot_graph = True)
    elif sym == 'BANKNIFTY':
        save_to_sheet(sym='BANKNIFTY', exp='26-Jun-24', df=df_banknifty, sheet_name='BANKNIFTY', wb=wb, plot_graph = True)
    elif sym == 'FINNIFTY':
        save_to_sheet(sym='FINNIFTY', exp='25-Jun-24', df=df_finnifty, sheet_name='FINNIFTY', wb=wb, plot_graph = True)
    else:
        save_to_sheet(sym='MIDCPNIFTY', exp='24-Jun-24', df=df_midcpnifty, sheet_name='MIDCPNIFTY', wb=wb, plot_graph = True)
#
#     # save_to_sheet(sym, exp, df_banknifty, 'banknifty', wb, plot_graph = True)
#     # # filename = f'test_intraday_overnight_with_chart_{sym}_{today_date}_exp_{exp}.xlsx'
#     # save_to_sheet(sym, exp, df_finnifty, 'finnifty', wb, plot_graph = True)
#     # # filename = f'test_intraday_overnight_with_chart_{sym}_{today_date}_exp_{exp}.xlsx'
#     # save_to_sheet(sym, exp, df_midcpnifty,'midcpnifty', wb, plot_graph = True)
# # filename = f'test_intraday_overnight_with_chart_{sym}_{today_date}_exp_{exp}.xlsx'
# save_to_sheet(sym = 'NIFTY', exp = '20-Jun-24', df=df_nifty, sheet_name='nifty', wb=wb, plot_graph = True)
# save_to_sheet(sym='BANKNIFTY', exp='26-Jun-24', df_banknifty, 'banknifty', wb, plot_graph = True)
# save_to_sheet(sym, exp, df_finnifty, 'finnifty', wb, plot_graph = True)
# save_to_sheet(sym, exp, df_midcpnifty, 'midcpnifty', wb, plot_graph = True)
filename = f'test_intraday_overnight_with_chart_{sym}_{today_date}_exp_{exp}.xlsx'
wb.save(filename)
