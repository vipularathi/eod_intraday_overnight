...
# ----------------------------------------------------------------
    ...
    # ----------------------------------------------------------------
        ...
        # trace1 = go.Scatter(x=df_filtered['time'], y=df_filtered['forward'], mode='lines', name='Forward', yaxis='y1')
        # trace2 = go.Scatter(x=df_filtered['time'], y=df_filtered['final_pnl'], mode='lines', name='Final PnL', yaxis='y2')
        # trace3 = go.Scatter(x=df_filtered['time'], y=df_filtered['use_strike'], mode='lines', name='Use Strike', yaxis='y1')
        #
        # # Combine the traces
        ...
        # layout = go.Layout(
        #     title=f'{sym.upper()} Straddle ({exp.upper()} Expiry)',
        ...
        # # pio.write_image(fig, f'{sym.upper()} Straddle ({exp.upper()} Expiry).png')
        # img_bytes = pio.to_image(fig, format='png')
        #
        # # Add the image to the Excel sheet
        # # img = Image(f'{sym.upper()} Straddle ({exp.upper()} Expiry).png')
        # img = Image(img_bytes)
        # img.anchor = 'A1'
        # ws.add_image(img)
        ...
        # fig.add_trace(go.Scatter(x=np.array(filtered_df['time'].values), y=np.array(filtered_df['final_pnl'].values), name='Final PnL'))
        # fig.add_trace(go.Scatter(x=np.array(filtered_df['time'].values), y=np.array(filtered_df['forward'].values), name='Forward'))
        # fig.update_layout(title_text=f'{sym} Straddle ({exp})', xaxis_title='Time', yaxis_title='PnL')
        #
        ...
        # img_bytes = pio.to_image(fig, format='png')
        #
        # # Add the chart image to the Excel worksheet
        # img = Image(img_bytes)
        # img.anchor = 'A2'
        # ws.add_image(img)
